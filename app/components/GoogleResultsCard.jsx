...
/**
 * GoogleResultsCard — compact Google SERP replica
 */

export default function GoogleResultsCard({ data }) {
  if (!data) return null;
  ...
  return (
    <div className="google-card">
      ...
      <div className="google-search-bar">
        <svg className="google-logo" viewBox="0 0 272 92" width="74" height="24">
        ...
        <div className="google-search-input">
          <span className="google-query">{data.query}</span>
          <svg viewBox="0 0 24 24" width="20" height="20" className="google-search-icon">
      ...
      <div className="google-results-list">
        {data.results?.map((result, i) => (
          <div key={i} className="google-result">
            <div className="google-result-url">
              <span className="google-result-favicon">
                {getDomainFavicon(result.url)}
              </span>
              <div className="google-result-url-text">
                <span className="google-result-site">{getDomainName(result.url)}</span>
                <span className="google-result-path">{result.url}</span>
              </div>
            </div>
            <h3 className="google-result-title">{result.title}</h3>
            <p className="google-result-snippet">{result.snippet}</p>
          </div>
        ))}
      </div>
    </div>
...
function getDomainFavicon(url) {
  ...
  return (
    <span className="google-favicon-circle">{initial}</span>
  );
}
